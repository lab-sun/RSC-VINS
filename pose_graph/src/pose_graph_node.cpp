#include "keyframe.h"
#include "netvald_loop/netvlad_detector.h"
#include "netvald_loop/netvlad_inference.h"
  ...
  while (true) {
    ...
    if (pose_msg != NULL) {
      ...
      if ((T - last_t).norm() > SKIP_DIS) {
        ...
        cv::Mat input_image = keyframe->image;
        std::string pkg_path = ros::package::getPath("pose_graph");
        std::string input_image_path = pkg_path + "/src/netvald_loop/tmp/img.png";
        if (!cv::imwrite(input_image_path, input_image)) {
          throw std::runtime_error("Failed to write image to disk");
        }
        std::vector<float> descriptor = inference_wrapper.infer_image(input_image_path);
        int net_loop_index = -1;
        posegraph.netvlad_detector_->Process(frame_index, input_image, descriptor, T,
                                             &net_loop_index);
        auto end_time = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed_time = end_time - start_time;
        // printf("Time to per total netvlad %f seconds\n", elapsed_time.count());
        posegraph.addKeyFrame(keyframe, 1, net_loop_index);
        m_process.unlock();
        frame_index++;
        last_t = T;
      }
...
int main(int argc, char** argv) {
  ...
  if (LOOP_CLOSURE) {
    ...
    fsSettings["netvlad_model_path"] >> NETVLAD_MODEL_PATH;
    printf("netvlad_model_path %s \n", NETVLAD_MODEL_PATH.c_str());
