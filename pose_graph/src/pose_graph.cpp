...
PoseGraph::PoseGraph() {
  ...
  // Netvald
  netvlad_detector_ = std::make_shared<NetvladDetector>(0.8);
}
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop, int external_loop_index) {
  // shift to base frame
  Vector3d vio_P_cur;
  Matrix3d vio_R_cur;
  ...
  int loop_index = -1;
  if (flag_detect_loop) {
    if (use_netvlad_loop_) {
      loop_index = external_loop_index;
    } else {
  ...
  if (SAVE_LOOP_PATH) {
    ...
    loop_path_file << P.x() << " " << P.y() << " " << P.z() << " " << Q.x() << " " << Q.y() << " "
                   << Q.z() << " " << Q.w() << endl;
  }
