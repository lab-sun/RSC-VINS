...
Estimator::Estimator() : f_manager{Rs} {
  ...
  clearState();
  //   netvlad_initializer_->LoadDescriptors(AREA_ID.c_str());
}
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>>& image,
                             const std_msgs::Header& header, const std::vector<float>& descriptor) {
  ...
  if (solver_flag == INITIAL) {
    if (use_netvlad_init_) {
      Pose result_pose;
      bool success = netvlad_initializer_->QueryTopK(descriptor, 3, result_pose);
      if (success) {
        continue_retrieve_++;
        printf("-----------------------------\n");
        printf("result_trans is %f %f %f\n", result_pose.tx, result_pose.ty, result_pose.tz);
        Ps[frame_count] = Eigen::Vector3d(result_pose.tx, result_pose.ty, result_pose.tz);
        Rs[frame_count] =
            Eigen::Quaterniond(result_pose.qw, result_pose.qx, result_pose.qy, result_pose.qz)
                .toRotationMatrix();
        auto iter_frame = all_image_frame.find(header.stamp.toSec());
        iter_frame->second.R =
            Eigen::Quaterniond(result_pose.qw, result_pose.qx, result_pose.qy, result_pose.qz)
                .toRotationMatrix();
        iter_frame->second.T = Eigen::Vector3d(result_pose.tx, result_pose.ty, result_pose.tz);
        printf("Pose Matching Success!\n");
      } else {
        printf("Pose Matching Failed!\n");
      }
      ...
      if (frame_count == WINDOW_SIZE) {
        ...
        if (result) {
          ...
          last_R0 = Rs[0];
          last_P0 = Ps[0];
          printf("last_R is %f %f %f\n", last_R(0, 0), last_R(1, 0), last_R(2, 0));
          printf("last_P is %f %f %f\n", last_P(0), last_P(1), last_P(2));
          printf("last_R0 is %f %f %f\n", last_R0(0, 0), last_R0(1, 0), last_R0(2, 0));
          printf("last_P0 is %f %f %f\n", last_P0(0), last_P0(1), last_P0(2));
        } else
          slideWindow();
      } else
        frame_count++;
...
bool Estimator::InitialByNetvlad() {
  TicToc t_sfm;
  // check imu observibility Todo!
