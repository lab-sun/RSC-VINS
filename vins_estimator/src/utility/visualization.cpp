...
void printStatistics(const Estimator &estimator, double t) {
  ...
  // estimator.Ps[WINDOW_SIZE].z()); ROS_DEBUG_STREAM("position: " <<
  // estimator.Ps[WINDOW_SIZE].transpose()); ROS_DEBUG_STREAM("orientation: " <<
  // estimator.Vs[WINDOW_SIZE].transpose());
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header) {
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
    ...
    foutC << estimator.Ps[WINDOW_SIZE].x() << " " << estimator.Ps[WINDOW_SIZE].y() << " "
          << estimator.Ps[WINDOW_SIZE].z() << " " << tmp_Q.x() << " " << tmp_Q.y() << " "
          << tmp_Q.z() << " " << tmp_Q.w() << endl;
    foutC.close();
  }
}
