...
void process() {
  estimator.netvlad_initializer_ = std::make_shared<NetVladInitializer>(PRIOR_MAP_PATH.c_str());
  ImageInferenceWrapper inference_wrapper(NETVLAD_MODEL_PATH.c_str());
  estimator.netvlad_initializer_->LoadDescriptors(AREA_ID.c_str());
  while (true) {
    std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>>
        measurements;
    ...
    for (auto& measurement : measurements) {
      ...
      m_buf.lock();
      cv_bridge::CvImageConstPtr ptr = nullptr;
      sensor_msgs::ImageConstPtr image_msg = NULL;
      double target_time = img_msg->header.stamp.toSec();
      bool has_right_img = false;
      while (!image_buf.empty()) {
        double image_time = image_buf.front()->header.stamp.toSec();
        if (image_time < target_time) {
          image_buf.pop();
        } else if (image_time == target_time) {
          image_msg = image_buf.front();
          image_buf.pop();
          has_right_img = true;
          //   ROS_INFO("find the right image with time %f", image_time);
          break;
        } else {
          ROS_WARN("image time %f is later than target time %f", image_time, target_time);
          break;
        }
      }
      if (has_right_img) {
      ...
      if (estimator.solver_flag == Estimator::SolverFlag::INITIAL) {
        ...
        std::string input_image_path = pkg_path + "/src/netvlad_initial/data/img.png";
        if (!cv::imwrite(input_image_path, raw_img)) {
          throw std::runtime_error("Failed to write image to disk");
        }
        descriptor = inference_wrapper.infer_image(input_image_path);
      }
      estimator.processImage(image, img_msg->header, descriptor);
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber sub_raw_image = n.subscribe(IMAGE_TOPIC, 2000, image_callback);
